        ...
        <c:forEach items="${pvs}" var="pv" varStatus="st" >
            <div style="flex: 0 0 20%;margin: 10px">
                ...
                <input type="text" class="form-control propertyValue" value="${pv.value}"
                       name="/admin/propertyValue/update?id=${pv.id}" id="${pv.id}" />
            </div>
        </c:forEach>
    </div>
    <div class="row clearfix">
        <div class="col-md-12 column">
            <c:if test="${pts.size()!=0}">
                <form role="form" action="/admin/propertyValue/add" method="post">
                    <div class="form-group">
                        <input type="hidden" class="form-control" name="pid" value="${product.id}" />
                    </div>
                    <div class="form-group">
                        <label for="property">添加属性:</label>
                        <select class="form-control" id="property" name="ptIdName">
    ...
    <div class="row clearfix">
        <div class="col-md-12 column">
            <c:if test="${pvs.size()!=0}">
                <form role="form" action="/admin/propertyValue/delete" method="post">
                    <div class="form-group">
                        <label for="delPv">选择删除属性:</label>
                        <select class="form-control" id="delPv" name="pvIdValue">
                            <c:forEach items="${pvs}" var="pv" varStatus="st" >
