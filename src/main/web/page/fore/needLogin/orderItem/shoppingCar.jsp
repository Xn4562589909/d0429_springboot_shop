        ...
        <div class="col-md-12 column">
            <nav class="navbar navbar-default navbar-inverse" role="navigation">
                <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                    ...
                    <form class="navbar-form navbar-left" role="search" action="/fore/foreProduct/nameList"
                          method="get">
            ...
            <c:if test="${orderItems.size()!=0}">
                <table class="table">
                    ...
                    <c:forEach items="${orderItems}" var="orderItem" varStatus="st">
                        <tr class="warning">
                            <td>
                                <a href="/fore/foreProduct/show?id=${orderItem.product.id}">
                                    ...
                                    <div class="input-group">
			                            <span class="input-group-btn">
			                                <button class="btn btn-info add" type="button"
                                                    name="/fore/orderItem/addProductNum?id=${orderItem.id}">+</button>
			                            </span>
                                        ...
                                        <span class="input-group-btn">
				                            <button class="btn btn-info sub" type="button"
                                                    name="/fore/orderItem/subProductNum?id=${orderItem.id}">-</button>
				                        </span>
                                    </div><!-- /input-group -->
                                </div></td>
                            ...
                            <td>
                                <button type="button" class="btn btn-default btn-danger delete-btn"
                                        name="/fore/orderItem/delete?id=${orderItem.id}">删除
                                </button>
                            </td>
                        </tr>
                    </c:forEach>
...
<script>
    $(function () {
        ...
        $("#pay").click(function () {
            ...
            $.ajax({
                type:'POST',
                url:'/fore/orderItem/carBuy',
                data:{list:array},
                traditional:true,
                success:function (result) {
                    // console.log(result);
                    window.location.href='/fore/orderItem/goOrder?id='+result;
                },
                error:function () {
                    alert("系统异常,接口出错了")
                }
